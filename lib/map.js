...
GameMap.prototype.neutralsOnAllyBorder = function neutralsOnAllyBorder () {
  var result = []
  for (var i = 0; i < this.regions.length; ++i) {
    var region = this.regions[i]
    if (region.owner === players.neutral && region.bordersAlly()) {
      result.push(region)
    }
  }
  return result
...
GameMap.prototype.handleUpdate = function handleUpdate (data) {
  ...
  for (var j = 0; j < this.regions.length; ++j) {
    ...
    region.cache = {}
    region.reinforcement = 0
    if (handled.indexOf(region.id) === -1 && region.owner === players.us) {
      region.owner = players.them
    }
  }
  for (var k = 0; k < this.superRegions.length; ++k) {
    this.superRegions[k].cache = {}
  }
}
...
SuperRegion.prototype.percentOwned = function percentOwned () {
  if (this.cache.percentOwned === undefined) {
    var count = 0
    for (var i = 0; i < this.regions.length; ++i) {
      if (this.regions[i].owner === players.us) {
        count += 1
      }
    }
    this.cache.percentOwned = count / this.regions.length
  }
  return this.cache.percentOwned
}

SuperRegion.prototype.getBadTroopCount = function getBadTroopCount () {
  if (this.cache.badTroopCount === undefined) {
    this.cache.badTroopCount = 0
    for (var i = 0; i < this.regions.length; ++i) {
      var region = this.regions[i]
      if (region.owner !== players.us) {
        this.cache.badTroopCount += region.troopCount
      }
    }
  }
  return this.cache.badTroopCount
}
...
Region.prototype.bordersAlly = function bordersAlly () {
  return this.getNeighboringAllies().length > 0
}
