...
Bot.prototype.attackNeutrals = function attackNeutrals () {
  ...
  for (var i = 0; i < possibleTargets.length; ++i) {
    ...
    this.log(
      'Neutral SuperRegion: %s, Region: %s, Value %s',
      targetRegion.superRegion.id,
      targetRegion.id,
...
/**
 * This is what sorts the neutral territories to be attacked. I think
 * I should first sort by super region value, then sort by the regions
 * in that super region and attack the one with the least troops. Also
 * trying to avoid enemy borders.
 */

function calcNeutralValue (region) {
  var value = Math.floor(
      ((region.superRegion.bonus / region.superRegion.getBadTroopCount()) * 100) +
      -(region.troopCount * 10) +
      -(region.getNeighboringEnemies().length)
  )
...
Bot.prototype.handleStartingRegions = function handleStartingRegions (ids) {
  ...
  for (var i = 0; i < ids.length; i++) {
    ...
    var value = 100
    value -= region.superRegion.getBadTroopCount() * 5
