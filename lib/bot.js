...
Bot.prototype.handleOpponentMoves = function handleOpponentMoves () {
  var duration = process.hrtime(this.last)

  var micros = Math.floor(duration[0] * 1000000 + duration[1] / 1000)
  if (this.round !== 0) {
    this.log('Round %s end, took %s µs', this.round, micros)
  } else {
    this.log('Init took %s µs', micros)
  }
  this.round += 1
